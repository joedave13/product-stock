...
<div class="container mt-3">
    <div class="card">
        <div class="card-header">
            <h4>Edit Transaksi Pengeluaran</h4>
        </div>
        <div class="card-body">
            <?php
                include '../koneksi.php';
                $id = $_GET['id'];
                $pengeluaran = mysqli_query($koneksi, "SELECT * FROM pengeluaran WHERE id = '$id'");
                while($p = mysqli_fetch_array($pengeluaran)){
            ?>
            <form action="pengeluaran_edit_act.php" method="post">
                <input type="hidden" name="id" id="id" value="<?= $p['id']; ?>">
                <div class="form-group">
                    <label for="kode">Kode Transaksi</label>
                    <input type="text" name="kode" id="kode" class="form-control" value="<?= $p['kode_pengeluaran']; ?>"
                        readonly>
                </div>
                <div class="form-group">
                ...
                <table class="table table-bordered table-striped" id="tbl_barang">
                    ...
                    <tbody>
                        <?php 
                            $id_dokumen = $p['id'];
                            $barang = mysqli_query($koneksi, "SELECT * FROM pengeluaran_detail WHERE id_pengeluaran = '$id_dokumen'");
